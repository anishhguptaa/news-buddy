from crewai import Task
from tools import search_tool
from agents import news_researcher_agent, news_writer_agent

news_researcher_task = Task(
  description=(
    "Identify the next big trend or latest news in {topic}. Focus on identifying pros and cons and the overall narrative. Your final report should clearly articulate the key points, its market opportunities, and potential risks."
  ),
  expected_output='A comprehensive 3 paragraphs long report.',
  tools=[search_tool],
  agent=news_researcher_agent,
)


news_writer_task = Task(
  description=(
    "Compose an insightful article on {topic}. Focus on the latest trends and how it's impacting the industry. This article should be easy to understand, engaging, and positive."
  ),
  expected_output='A 4 paragraph article on {topic} advancements formatted as markdown.',
  tools=[search_tool],
  agent=news_writer_agent,
  output_file='./news/{topic}_news.md'
)